          ...
          {educationData.map((item, idx) => (
            <div key={idx} className="flex group relative">
              ...
              <div className="flex flex-col items-center mr-4 w-6 shrink-0 relative">
                <div className={`w-[2px] grow bg-zinc-200 dark:bg-zinc-800 ${idx === 0 ? "mt-8" : ""} ${idx === educationData.length - 1 ? "mb-auto h-8" : ""}`}></div>
                <div className="absolute top-6 w-4 h-4 rounded-full border-2 border-emerald-500 bg-white dark:bg-zinc-950 z-10 group-hover:bg-emerald-500 transition-colors duration-300"></div>
              </div>
              ...
              <div className="pb-12 flex items-start flex-1 gap-4">
                ...
                <div className="relative w-full p-6 rounded-xl border bg-zinc-50/80 border-zinc-100 dark:bg-zinc-900/70  dark:border-zinc-800 shadow-md transition-all duration-300 hover:border-emerald-500/30 hover:shadow-lg">
                  ...
                  {item.highlight && (
                    <div className="mb-4 inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gradient-to-r from-amber-500/10 via-orange-500/10 to-amber-500/10 border border-amber-500/20 shadow-[0_0_15px_rgba(245,158,11,0.2)] dark:shadow-[0_0_20px_rgba(245,158,11,0.15)]">
                      <Trophy size={16} className="text-amber-500 fill-amber-500" />
                      <span className="text-sm font-bold bg-gradient-to-r from-amber-600 to-orange-600 dark:from-amber-400 dark:to-orange-400 bg-clip-text text-transparent font-1spaceGrotesk">
                        {item.highlight}
                      </span>
                    </div>
                  )}

                  <p className="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed font-1spaceGrotesk">
                    {item.description}
                  </p>
                </div>
              </div>
